...
def get_sliced_prediction(
    ...
    slice_image_result = slice_image(
        ...
        output_file_name=os.path.basename(image)[:-4],
        output_dir=r"/work/data/855G/tmp",
        out_ext=".bmp"
    )
    ...
    if postprocess_type in POSTPROCESS_NAME_TO_CLASS.keys():
    ...
    for group_ind in range(num_group):
        ...
        if merge_buffer_length is not None and len(object_prediction_list) > merge_buffer_length and postprocess:
            object_prediction_list = postprocess(object_prediction_list)
    ...
    if len(object_prediction_list) > 1 and postprocess:
        object_prediction_list = postprocess(object_prediction_list)
